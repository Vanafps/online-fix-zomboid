...
function MultiplayerUI:fetchWebServers()
    local ok, err = pcall(function()
        local reader = getFileReader("servers.json", true)
        if not reader then
            print("fetchWebServers: C:\\Users\\DomPC\\Zomboid\\servers.json not found")
            return
        end
        ...
        reader:close()
        print("fetchWebServers: successfully read servers.json, length: " .. tostring(string.len(jsonStr)))
        
        -- Parse JSON manually
        local pos = 1
        while true do
            local startIdx = string.find(jsonStr, "{", pos)
            if not startIdx then break; end
            local endIdx = string.find(jsonStr, "}", startIdx)
            if not endIdx then break; end
            
            local block = string.sub(jsonStr, startIdx, endIdx)
            
            -- Extract name
            local name = nil
            local _, nameStart = string.find(block, '"name"%s*:%s*"')
            if nameStart then
                local nameEnd = string.find(block, '"', nameStart + 1)
                if nameEnd then
                    name = string.sub(block, nameStart + 1, nameEnd - 1)
                end
            end
            
            -- Extract ip
            local ip = nil
            local _, ipStart = string.find(block, '"ip"%s*:%s*"')
            if ipStart then
                local ipEnd = string.find(block, '"', ipStart + 1)
                if ipEnd then
                    ip = string.sub(block, ipStart + 1, ipEnd - 1)
                end
            end
            
            -- Extract port
            local port = nil
            local _, portStart = string.find(block, '"port"%s*:%s*')
            if portStart then
                local portEnd = string.find(block, '[,%s}]', portStart + 1)
                if portEnd then
                    port = tonumber(string.sub(block, portStart + 1, portEnd - 1))
                end
            end
            
            if name and ip and port then
                -- Check if already exists in self.serverList
                local found = false
                for i, v in ipairs(self.serverList) do
                    if v:getIp() == ip and v:getPort() == port then
                        found = true
                        break
                    end
                end
                ...
                if not found then
                    ...
                    newServer:setPlayers("0")
                    newServer:setMaxPlayers("32")
                    newServer:setOpen(true)
                    newServer:setPublic(true)
                    newServer:setResponded(false)
                    newServer:setVersion(getCore():getVersionNumber())
                    newServer:setPasswordProtected(false)
                    -- Do NOT set steamID to force raw UDP
                    table.insert(self.serverList, newServer)
                end
            end
    ...
    if getSteamModeActive() then
        for i, v in ipairs(self.serverList) do
            local hasNoSteamId = true
            if v.getSteamId then
                local sid = v:getSteamId()
                if sid and sid ~= "" then
                    hasNoSteamId = false
                end
            end
            if hasNoSteamId and v:getIp() and v:getIp() ~= "" and v:getPort() then
                steamRequestServerDetails(getHostByName(v:getIp()), v:getPort())
            end
        end
    end
    
    if not ok then
        print("fetchWebServers error: " .. tostring(err))
    end
end
...
function MultiplayerUI:sortInternetList()
    -- Check if we need to initialize servers.txt with default servers
    local reader = getFileReader("servers.txt", true)
    ...
    -- Read custom servers from servers.txt
    reader = getFileReader("servers.txt", true)
    if reader then
        local line = reader:readLine()
        while line do
            ...
            if line ~= "" and string.sub(line, 1, 1) ~= "#" then
                ...
                if colonIdx then
                    local ip = string.sub(line, 1, colonIdx - 1)
                    local portStr = string.sub(line, colonIdx + 1)
...
function MultiplayerUI.OnSteamServerResponded2(host, port, server2)
    print('OnSteamServerResponded2: ' .. host .. ':' .. tostring(port) .. ' responded, ping: ' .. tostring(server2:getPing()) .. ', players: ' .. tostring(server2:getPlayers()))
    ...
    -- Update server list (public internet tab)
    local updatedAny = false
    for i, v in ipairs(self.serverList) do
        if (v:getIp() == host or v:getIp() == server2:getIp()) and v:getPort() == port then
            v:setResponded(true)
            v:setPing(server2:getPing())
            v:setPlayers(server2:getPlayers())
            v:setMaxPlayers(server2:getMaxPlayers())
            v:setMods(server2:getMods())
            v:setPasswordProtected(server2:isPasswordProtected())
            updatedAny = true
        end
    end
...
function MultiplayerUI.OnSteamServerFailedToRespond2(host, port)
    print('OnSteamServerFailedToRespond2: ' .. host .. ':' .. tostring(port) .. ' failed to respond')
    local self = MultiplayerUI.instance
    if not self then return end
    ...
    local updatedAny = false
    for i, v in ipairs(self.serverList) do
        if v:getIp() == host and v:getPort() == port then
            v:setResponded(false)
            updatedAny = true
        end
    end
